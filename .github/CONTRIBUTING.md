# Contributing to `markdown-decorator`
...
## 6. Development
`markdown-decorator` is dedicated to enabling best practices. It is therefore crucial that the generator itself follows strong engineering standards to assure quality and performance.

### 6.1. Code standards

ESLint evaluates Javascript code standards before each test run with the `npm-script pretest`.
Heres' a [table of `markdown-decorator's` ESLint rules][eslint-rules-table-url] and their enforcement.
...
[issues-new-url]: https://github.com/gregswindle/markdown-decorator/issues/new
